...
##############################################################################
# FORCE C++14 & CMAKE POLICIES
##############################################################################
set (CMAKE_CXX_STANDARD 14)
SET(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -DBOOST_SYSTEM_NO_DEPRECATED -fPIC -std=c++14 -pthread")
...
function(set_compiler_flags target)
    if(NOT MSVC)
        set_gcc_flags()
        target_compile_options(${target} PRIVATE "${MONKEY_COMPILER_FLAGS}")
        string(REPLACE ";" " " _TMP "${MONKEY_COMPILER_FLAGS}")
...
function(set_compiler_warnings target)
    if(NOT MSVC)
        set_gcc_warnings()
        target_compile_options(${target} PRIVATE "${MONKEY_COMPILER_WARNINGS}")
    endif()
endfunction()


macro(set_gcc_flags)
#    list(APPEND MONKEY_COMPILER_FLAGS "-fno-exceptions" "-std=c++${CPP_STANDARD}")
...
macro(set_gcc_warnings)
    list(APPEND MONKEY_COMPILER_WARNINGS "-Wall" "-Werror" "-pedantic" "-Wextra")
    if(COMPILER_CLANG)
        if(PLATFORM_ANDROID)
            ...
            if(ANDROID_STL MATCHES "libc" AND ANDROID_NATIVE_API_LEVEL_NUM LESS "21")
                ...
                if(NDK_RELEASE_NUMBER LESS "12000")
                    string(REGEX REPLACE "-pedantic" "" MONKEY_COMPILER_WARNINGS "${MONKEY_COMPILER_WARNINGS}")
                else()
                    list(APPEND MONKEY_COMPILER_WARNINGS "-Wno-gnu-include-next")
                endif()
            endif()
        endif()
    endif()
